...
pam_I=pam(1:length(pam)/2);   % seguencia I (en fase) e Q (em quadratura)
pam_Q=pam(length(pam)/2+1:length(pam)); % dos simbolos QAM
...
ISI_channel = [0.19+.59j .45-1.28j -.14-.53j -.19+.23j .33+.51j];
sinal_rx_ISI=filter(ISI_channel,1,sinal_QAM);
sinal_rx_ISI = awgn(sinal_rx_ISI,snr,'measured');
...
% Densidade Espectral de Potência do sinal recebido
[Pxx,F]=pwelch(sinal_rx_ISI/max(abs(sinal_rx_ISI)),[],[],[],Fs,'twosided');
plot((F-Fs/2)/1000, 10*log10(fftshift(Pxx)));
...
% -------------------------------------------------------------------------
sinal_rx_casado=conv(sinal_rx_ISI,filtro);       %Filtro casado

sinal_rx_casado_I=conv(real(sinal_rx_ISI),filtro); %Filtro casado em fase
sinal_rx_casado_Q=conv(imag(sinal_rx_ISI),filtro); %Filtro casado em Quadratura
...
%-------------------------------
symbols_rx_I=a(symbols_rx_I);
symbols_rx_Q=a(symbols_rx_Q);          %Reconstrucao da imagem no receptor
bit_symbols_rx_I=de2bi(symbols_rx_I);
bit_symbols_rx_Q=de2bi(symbols_rx_Q);
bit_matrix_rx=reshape([bit_symbols_rx_I;bit_symbols_rx_Q], im_size, L);
